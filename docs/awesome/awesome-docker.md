 ...
 **_如果您在这里看到的链接（不再）适合，您可以通过提交 [pull request][editreadme] 来修复它以改进此文件. 谢谢！_**

此列表的创建者和维护者不会收到任何形式的付款来接受任何贡献者所做的更改. 该页面在任何方面都不是官方的 Docker 产品. 它是一个项目链接列表，由志愿者维护. 欢迎大家踊跃投稿. 这个 repo 的目标是索引开源项目，而不是为了盈利而做广告.

所有链路都经过家庭烘焙监控和测试 [Node.js script](https://github.com/veggiemonk/awesome-docker/blob/master/pull_request.js)
...
- [Docker for novices](https://www.youtube.com/watch?v=xsjSadjKXns) 面向从未使用过 Docker 的开发人员和测试人员的 Docker 介绍.  （视频 1 小时 40 分，录制于 linux.conf.au 2019 — 新西兰基督城），作者是 Alex Clews.
...
- [The Docker Handbook](https://docker-handbook.farhan.dev/) 一本开源书籍，教您基础知识、最佳实践和一些中级 Docker 功能. 该书托管于 [fhsinchy/the-docker-handbook](https://github.com/fhsinchy/the-docker-handbook) 项目托管在 [fhsinchy/docker-handbook-projects](https://github.com/fhsinchy/docker-handbook-projects) 存储库.
...
- [docker-config-update](https://github.com/sudo-bmitch/docker-config-update) - 用于更新 docker 配置和机密的实用程序，以便在撰写文件中进行部署 [@sudo-bmitch](https://github.com/sudo-bmitch)
...
- [Stacker](https://github.com/stacker/stacker-cli)  :skull: - Docker Compose 模板.  Stacker 在 Docker Compose 上提供了一个抽象层和更好的 DX（开发人员体验）.
- [Stitchocker](https://github.com/alexaandrov/stitchocker)  - 一个轻量级和快速的命令行实用程序，用于方便地对 docker-compose 多个容器服务进行分组. 经过 [@alexaandrov](https://github.com/alexaandrov)
...
- [Clocker](https://github.com/brooklyncentral/clocker)  - Clocker 创建和管理 Docker 云基础架构.  Clocker 支持多节点应用程序的单击部署和运行时管理，这些应用程序作为分布在多个主机上的容器在 Docker 和 Marathon 上运行. 它利用 [Calico][calico] 和 [Weave][weave] 进行网络和 [Brooklyn](https://brooklyn.apache.org/) 用于应用程序蓝图. 经过 [@brooklyncentral](https://github.com/brooklyncentral)
...
- [SwarmManagement](https://github.com/DIPSAS/SwarmManagement)  - Swarm Management 是一个 Python 应用程序，使用 pip 安装. 该应用程序通过配置一个 yaml 文件来描述要部署的堆栈以及要创建的网络、配置或机密，从而可以轻松地管理 Docker Swarm.
...
- [InfluxDB, cAdvisor, Grafana](https://github.com/vegasbrianc/docker-monitoring) - InfluxDB 时间序列 DB 与 Grafana 和 cAdvisor 相结合 [@vegasbrinc][vegasbriac]
...
- [monit-docker](https://github.com/decryptus/monit-docker)  - 监控 docker 容器资源使用情况或状态并执行 docker 命令或在容器内执行.  [@decryptus][解密]
- [NexClipper](https://github.com/NexClipper/NexClipper) - NexClipper 是专门针对 Docker、Apache Mesos、Marathon、DC/OS、Mesosphere、Kubernetes 的容器监控和性能管理解决方案，由 [@Nexclipper](https://github.com/NexClipper)
...
- [Pipework](https://github.com/jpetazzo/pipework)  - 用于 Linux 容器的软件定义网络，Pipework 可与“普通”LXC 容器以及出色的 Docker 一起使用. 由 [@jpetazzo][jpetazzo]
...
- [Marathon](https://github.com/mesosphere/marathon)  - Marathon 是一个建立在 Mesos 上的私有 PaaS. 它会自动处理硬件或软件故障并确保应用程序“始终在线”
...
- [Rancher](https://github.com/rancher/rancher) - 一个开源项目，为 [@rancher][rancher] 在生产中运行 Docker 提供了一个完整的平台.
- [Swarm-cronjob](https://github.com/https://github.com/veggiemonk/awesome-docker/blob/master/crazy-max/swarm-cronjob) - 在 Swarm 上按基于时间的计划创建作业 [@https://github.com/veggiemonk/awesome-docker/blob/master/crazy-max](https://github.com/veggiemonk/awesome-docker/blob/master/crazy-max)
...
- [Dcw](https://github.com/pbertera/dcw) - Docker-compose SSH 包装器：一个非常可怜的人 PaaS，暴露了容器标签中定义的 docker-compose 和 custom-container 命令.
...
- [Hephy Workflow](https://github.com/teamhephy/workflow)  - Kubernetes 的开源 PaaS，为任何 Kubernetes 集群添加了一个对开发人员友好的层，使部署和管理应用程序变得容易. 叉 [Deis Workflow](https://github.com/deis/workflow)
- [Krane](https://github.com/krane/krane) - 用于管理远程服务器上的容器工作负载的工具集
- [Nanobox](https://github.com/nanobox-io/nanobox) :heavy_dollar_sign: - 一个应用程序开发平台，可以创建本地环境，然后可以在云中部署和扩展.
...
- [nginx-proxy][nginxproxy] - 使用 [@jwilder][jwilder] 使用 docker-gen 的 Docker 容器的自动 nginx 代理
...
- [Deepfence Enterprise](https://https://github.com/veggiemonk/awesome-docker/blob/master/deepfence.io)  :heavy_dollar_sign: - 适用于 kubernetes、虚拟机和无服务器的全生命周期云原生工作负载保护平台. 经过 [@https://github.com/veggiemonk/awesome-docker/blob/master/deepfence](https://github.com/veggiemonk/awesome-docker/blob/master/deepfence)
...
- [Prisma Cloud](https://www.paloaltonetworks.com/prisma/cloud) :heavy_dollar_sign: -（以前称为 Twistlock 安全套件）检测漏洞、强化容器映像并在应用程序的整个生命周期内实施安全策略.
...
- [ctop (1)](https://github.com/yadutaf/ctop) - :skull: 一个基于命令行/文本的 Linux Containers 监控工具，它的工作方式与您期望的一样 (Python) [@yadutaf](https://github.com/yadutaf)
...
- [lazydocker](https://github.com/jesseduffield/lazydocker)  - 管理一切 docker 的懒惰方式. 一个简单的 docker 和 docker-compose 终端 UI，用 Go 语言编写，使用 gocui 库. 经过 [@jesseduffield](https://github.com/jesseduffield)
...
- [ns-enter](https://github.com/jpetazzo/nsenter) - :skull: 不再使用 ssh，通过 [@jpetazzo][jpetazzo] 输入容器的命名空间
...
- [Container Web TTY](https://github.com/wrfly/container-web-tty) - 通过 web-tty 连接你的容器 [@wrfly](https://github.com/wrfly)
...
- [Kubevious](https://github.com/kubevious/kubevious) - Kubernetes 的高度可视化 Web UI，通过以应用程序为中心的方式呈现配置和状态 [@rubenhak](https://github.com/rubenhak).
...
- [amicontained](https://github.com/genuinetools/amicontained)  - 容器内省工具. 找出 [@genuinetools][genuinetools] 正在使用的容器运行时以及可用的功能
...
- [sue](https://github.com/theAkito/sue)  - 以与运行 sue 的用户不同的用户身份执行程序. 这是 ncopa/su-exec 的可维护替代方案，后者是更好的 tianon/gosu. 这个比原来的 gosu 好得多（性能更高，体积更小），但是比用纯 C 编写的 su-exec 更容易维护. [@theAkito](https://github.com/theAkito)
- [supercronic](https://github.com/aptible/supercronic) - 与 crontab 兼容的作业运行器，专门设计用于在容器中运行 [@aptible](https://github.com/aptible/)
...
- [cekit](https://github.com/cekit/cekit) - openshift 使用的一种工具，用于使用不同的构建引擎构建基础镜像 [@cekit](https://github.com/cekit).
...
- [chaperone-docker](https://github.com/garywiz/chaperone-docker) - 一组使用 Chaperone 流程管理器的图像，包括精益的 Alpine 图像、LAMP、LEMP 和基本工具包.
...
- [Dockerfile Project](https://dockerfile.github.io/)  - 值得信赖的自动化 Docker 构建.  Dockerfile 项目为可在 Docker 容器上运行的各种流行的开源软件服务维护 Dockerfile 的中央存储库.
...
- [Vektorcloud](https://github.com/vektorcloud) - 最小的、基于 Alpine 的 Docker 镜像的集合
...
- [docker-image-size-limit](https://github.com/wemake-services/docker-image-size-limit) - 一种监视泊坞窗图像大小的工具.
...
- [Hadolint](https://github.com/hadolint/hadolint)  - 一个 Dockerfile linter，用于检查最佳实践、常见错误，并且还能够对任何用 `RUN` 指令编写的 bash 进行 lint； 经过 [@lukasmartinelli](https://github.com/lukasmartinelli)
...
- [Amazon EC2 Container Registry :heavy_dollar_sign:](https://aws.amazon.com/ecr/) - Amazon EC2 Container Registry (ECR) 是一个完全托管的 Docker 容器注册表，可让开发人员轻松存储、管理和部署 Docker 容器映像.
...
- [Cloudsmith :heavy_dollar_sign:](https://cloudsmith.com/docker-registry/)  - 完全托管的包管理 SaaS，对公共和私有 Docker 注册表（以及许多其他注册表）提供一流的支持，包括 Kubernetes 生态系统的 Helm 图表. 有一个慷慨的免费层，并且对于开源也是完全免费的.
...
- [Portus](https://github.com/SUSE/Portus) - Docker 注册表 (v2) 的授权服务和前端由 [@SUSE](https://github.com/SUSE)
...
- [DoMonit](https://github.com/eon01/DoMonit) - 一个用于 Docker API 的简单 Docker 监控包装器
...
- [Diun](https://github.com/https://github.com/veggiemonk/awesome-docker/blob/master/crazy-max/diun) - 在 Docker 注册表中更新镜像或存储库时接收通知 [@https://github.com/veggiemonk/awesome-docker/blob/master/crazy-max](https://github.com/veggiemonk/awesome-docker/blob/master/crazy-max).
...
- [GOCD-Docker](https://github.com/gocd/gocd-docker) :skull: - 转至 docker 容器中的服务器和代理进行配置.
...
- [EnvCLI](https://github.com/EnvCLI/EnvCLI)  - 用特定于项目的 docker 容器替换您本地安装的 Node、Go 等. 经过 [@EnvCLI](https://github.com/EnvCLI)
...
- [Gebug](https://github.com/moshebe/gebug) - 通过无缝启用调试器和热重载功能，使 Dockerized Go 应用程序的调试变得超级简单的工具.
...
- [dgoss](https://github.com/aelsabbahy/goss/tree/master/extras/dgoss) - 一个快速的基于 YAML 的工具，用于验证 docker 容器.
...
- [Google Container Engine](https://cloud.google.com/kubernetes-engine/docs/) :heavy_dollar_sign: - 由 [Kubernetes][kubernetes] 提供支持的谷歌云计算上的 Docker 容器.
...
- [Red Hat CodeReady Workspaces](https://developers.redhat.com/products/codeready-workspaces/overview) - 用于快速应用程序开发的协作 Kubernetes 原生解决方案，可在 Red Hat OpenShift 上提供一致的开发人员环境，让任何拥有浏览器的人都可以在两分钟内贡献代码.
- [Red Hat OpenShift Dedicated](https://cloud.redhat.com/products/dedicated/) :heavy_dollar_sign: - 在 Amazon Web Services 和 Google Cloud 上完全托管的 Red Hat® OpenShift® 服务
...
- [AppDynamics](https://github.com/Appdynamics/docker-monitoring-extension) - Docker 监控扩展使用 Unix 套接字或 TCP 从 Docker 远程 API 收集指标.
...
- [Datadog](https://www.datadoghq.com/)  :heavy_dollar_sign: - Datadog 是一种用于大规模云环境的全栈监控服务，它聚合来自服务器、数据库和应用程序的指标/事件. 它包括对 Docker、Kubernetes 和 Mesos 的支持.
...
- [Docker Certification](https://intellipaat.com/docker-training-course/) :heavy_dollar_sign: 将帮助您通过动手项目和案例研究学习 Docker 容器化、运行 Docker 容器、映像创建、Dockerfile、Docker 编排、安全最佳实践等，并帮助您清除 Docker Certified Associate.
...
- [ToolsOfTheTrade](https://github.com/cjbarber/ToolsOfTheTrade) SaaS 和内部部署应用程序列表 [@cjbarber](https://github.com/cjbarber)
...
- [Webstack-micro](https://github.com/ferbs/webstack-micro) 演示 Web 应用程序展示了如何使用 Docker Compose 设置 API 网关、集中式身份验证、后台工作程序和作为容器化服务的 WebSockets.
...
[vegasbrianc]：https://github.com/vegasbriac
