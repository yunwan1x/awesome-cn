...
- [Duff](https://github.com/mirage/duff) -P. MacDonald在OCaml中实现了Rabin的指纹和增量压缩（与 [libXdiff](http://www.xmailserver.org/xdiff-lib.html)
- [ORaft](https://github.com/komamitsu/oraft) -的图书馆 [Raft consensus algorithm](https://raft.github.io/raft.pdf) 在OCaml中实现


## Application Libraries

- [Batteries Included](https://github.com/ocaml-batteries-team/batteries-included) –用于OCaml项目的由社区维护的基础库.
...
- [Base](https://github.com/janestreet/base) -Jane Street Capital的无依赖关系，快速编译，可在任何可以运行OCaml代码标准库的环境中完全移植.
...
- [easy-format](https://github.com/mjambon/easy-format) –用于OCaml的漂亮打印库.
...
- [More OCaml: Algorithms, Methods, and Diversions](https://www.amazon.com/More-OCaml-Algorithms-Methods-Diversions/dp/0957671113/)  –在More OCaml中，John Whitington进行了OCaml函数编程的曲折之旅，介绍了各种语言功能并描述了一些经典算法. 本书以一个处理PDF文件生成的大型示例作为结尾. 每章都有问题，以及有效的答案和提示.
...
- [Functional Programming in OCaml: A Principled Approach](https://www.cs.cornell.edu/courses/cs3110/2021sp/textbook/) -康奈尔大学OCaml的CS 3110函数式编程教科书.
...
- [Infer](https://github.com/facebook/infer) -Infer是适用于Java，C和Objective-C的静态分析器
...
- [Camelot](https://github.com/upenn-cis1xx/camelot) -Camelot是模块化且完全可配置的OCaml短绒棉布和样式检查器.
...
-**语言和编译器**：
  - [Caramel](https://caramel.run/) -Caramel是用于构建类型安全，可伸缩和可维护的应用程序的功能语言.
  - [cDuce](http://www.cduce.org/) -cDuce是一种具有创新功能的现代XML面向功能语言.
  - [Compcert C Compiler](http://compcert.inria.fr/) -这是一个C编译器，支持大多数ISO C90和C99 / ANSI C功能.
...
-**解析器和词法生成器**：
  ...
  - [Menhir](http://gallium.inria.fr/~fpottier/menhir/) – Menhir是用于OCaml的LR（1）解析器生成器.
...
-**绑定**
  - [Dbm](https://forge.ocamlcore.org/projects/camldbm/) —与NDBM / GDBM Unix“数据库”的绑定.
  ...
  - [mysql](http://ygrek.org.ua/p/ocaml-mysql/) –绑定到libmysqlclient以便与MySQL数据库交互.
  - [pgx](https://github.com/arenadotio/pgx) –一个纯OCaml PostgreSQL客户端库
.
...
-**新实现**
  ...
  价值观. 可选的即时LZ4值压缩或tokyocabinet后端.

-**叠加**
  - [Sequoia](https://github.com/andrenth/sequoia) -Sequoia是MySQL / MariaDB和PostgreSQL的类型安全查询生成器
  ...
  - [Caqti](https://github.com/paurkedal/ocaml-caqti) -合作线程访问关系数据
-**文章**：
...
- [OCaml: Learn & Code iOS app](https://apps.apple.com/app/ocaml-learn-code/id1547506826) -通过iPhone / iPad / Mac学习并执行OCaml代码.
- [Jupyter](https://github.com/akabe/ocaml-jupyter) – Jupyter笔记本电脑的OCaml内核.
- [utop](https://github.com/ocaml-community/utop) – OCaml的通用顶层，支持多行版本，历史记录，实时和上下文相关的完成，颜色等.
- [ocamlformat](https://github.com/ocaml-ppx/ocamlformat) -用于格式化OCaml代码的命令行工具.
- [ocamlbrowser](http://caml.inria.fr/pub/docs/manual-ocaml/browser.html)  –使用LablTk编写的源代码和已编译的界面浏览器. 包含在ocaml &lt;= 4.01的标准发行版中，以及包含在labml&gt; = 4.02的labltk中.
...
- [OCaml at Exercism](http://exercism.io/languages/ocaml)  –锻炼是您进行有关代码的深思熟虑的场所. 探索简单性，惯用语言功能和富有表现力的可读代码. [Solutions](https://github.com/exercism/xocaml).
...
- [Alt-Ergo](http://alt-ergo.lri.fr/) – Alt-Ergo是一个开源SMT求解器，致力于证明在程序验证的上下文中生成的数学公式.
...
- [slacko](https://github.com/Leonidas-from-XIV/slacko) – OCaml中用于Slack的简洁界面.
...
- [lablgtk](https://garrigue.github.io/lablgtk/) —用于OCaml的GTK2和GTK3绑定，具有各种高级功能来定义GUI.
...
- [ocaml-linenoise](https://github.com/ocaml-community/ocaml-linenoise) -独立的OCaml绑定到Linenoise，在OCaml中提供简单的高级readline功能.
...
- [mirage](https://github.com/mirage/mirage) -图书馆操作系统，可为各种云计算和移动平台上的安全，高性能网络应用程序构建统一内核
...
- [AMQP](https://github.com/andersfugmann/amqp-client) –用于Async和Lwt的AMQP客户端库.
...
-**工具和语言扩展**：
  ...
  - [Fan](http://zhanghongbo.me/fan/)  – Fan是OCaml的编译时元编程系统，最初是从Camlp4汲取灵感的. 它是OCaml和Lispy宏的组合. 它与OCaml共享相同的具体语法.
...
-**绑定**：
  - [Cordova plugins](https://github.com/dannywillems/ocaml-cordova-plugin-list)  –绑定到Cordova插件的列表. 可以访问OCaml中的本机设备组件，如加速度计，SMS，地理位置等.
...
-**构建工具**：
  ...
  - [dune](https://github.com/ocaml/dune) –适用于OCaml的可组合且自以为是的构建系统（以前的jbuilder）
...
-**图书馆**：
  ...
  - [SPOC](https://github.com/mathiasbourgoin/SPOC) -库和语法扩展，可将密集的计算卸载到并行加速器（多核CPU，GPU和其他与GPGPU框架兼容的加速器）中.
...
- [Humane-re](https://github.com/rgrinberg/humane-re) –人性化的尝试为90％的正则表达式提供简单的界面礼节由ocaml-re提供
...
- [nocrypto](https://github.com/mirleft/ocaml-nocrypto)  – ocaml-tls项目背后的小型加密库. 它被构建为易于使用，遵循功能性编程原理并且能够在基于Xen的unikernel中运行.
...
- [jsonm](http://erratique.ch/software/jsonm) —用于OCaml的非阻塞流式JSON编解码器.
...
- **构架**：
  ...
  - [webmachine](https://github.com/inhabitedtype/ocaml-webmachine)  –用于OCaml的REST工具包.  OCaml Web机是cohttp之上的一层，它实现了基于状态机的HTTP请求处理器. 它特别适合于编写RESTful API. 顾名思义，这是Webmachine项目的OCaml端口.
