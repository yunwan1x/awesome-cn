...
[Cybersecurity blue teams](https://en.wikipedia.org/wiki/Blue_team_(computer_security) ) 是识别信息技术系统中的安全缺陷、验证安全措施的有效性并监控系统以确保实施的防御措施在未来仍然有效的一组个人. 虽然不是排他性的，但这份清单严重偏向于 [Free Software](https://www.gnu.org/philosophy/free-sw.html) 项目和反对专有产品或公司服务. 对于攻击性的 TTP，请参阅 [awesome-pentest](https://github.com/fabacab/awesome-pentest).
...
- [Clevis](https://github.com/latchset/clevis) - 自动解密的可插拔框架，常作为Tang客户端使用.
...
* [Atheris](https://pypi.org/project/atheris/) - 基于 libFuzzer 的覆盖引导 Python 模糊测试引擎，支持 Python 代码的模糊测试以及为 CPython 编写的本机扩展.
...
- [Tang](https://github.com/latchset/tang)  - 将数据绑定到网络存在的服务器； 仅当客户端在某个（安全）网络上时才向客户端提供数据.
...
- [Gluu Server](https://gluu.org/) - 使用免费和开源软件云原生社区分发对 Web 和移动应用程序进行集中身份验证和授权.
...
- [Infection Monkey](https://www.guardicore.com/infectionmonkey/) - 开源漏洞和攻击模拟 (BAS) 平台，可帮助您验证现有控制并确定攻击者如何利用您当前的网络安全漏洞.
...
- [SpoofSpotter](https://github.com/NetSPI/SpoofSpotter) - 捕捉欺骗的 NetBIOS 名称服务 (NBNS) 响应并向电子邮件或日志文件发出警报.
...
- [Prelude SIEM OSS](https://www.prelude-siem.org/) - 具有悠久历史的开源、无代理 SIEM 和多个商业变体，具有安全事件收集、规范化和来自任意日志输入和众多流行监控工具的警报.
...
- [PSRecon](https://github.com/gfoss/PSRecon) - 用于分析远程 Windows 系统的类似 PSHunt 的工具，该工具还可以生成关于其发现的独立 HTML 报告.
...
- [Forager](https://github.com/opensourcesec/Forager) - 使用 Python3 构建的多线程威胁情报收集具有简单的基于文本的配置和数据存储，易于使用和数据可移植.
...
- [Vanguards](https://github.com/mikeperry-tor/vanguards) - 第 3 版洋葱服务守卫发现攻击缓解脚本（旨在最终包含在 Tor 核心中）.
...
- [Santa](https://github.com/google/santa) - 在 macOS 的允许/拒绝列表系统中跟踪淘气或漂亮的二进制文件.
...
- [CobaltStrikeScan](https://github.com/Apr4h/CobaltStrikeScan) - 扫描 Cobalt Strike 信标的文件或进程内存并解析其配置.
...
- [Sticky Keys Slayer](https://github.com/linuz/Sticky-Keys-Slayer) - 从主机名列表中建立 Windows RDP 会话并扫描辅助工具后门，如果发现后门，则发出警报.
