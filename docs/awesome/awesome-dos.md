...
- [Plutonium Caverns](https://jani-nykanen.itch.io/plutonium-caverns) - Overhead puzzle game written in C. Web version uses [DOSBox ported to Emscripten](https://github.com/dreamlayers/em-dosbox) <i>to embed DOSBox into HTML5.</i><b>将 DOSBox 嵌入 HTML5。</b> <i>However, the original executable is also downloadable and buildable with Open Watcom.</i><b>但是，原始可执行文件也可以通过 Open Watcom 下载和构建。</b>
...
- [Quake](https://github.com/id-Software/Quake) <i>- FPS developed by id Software set in a fully 3D world.</i> <b>- 由 id Software 开发的 FPS 设置在一个完全 3D 的世界中。</b> <i>Written in C. Compiled with DJGPP for DOS.</i><b>用 C 语言编写。用 DJGPP 为 DOS 编译。</b>
