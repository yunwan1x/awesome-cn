...
该存储库包含不同城市和区域规划数据和技术资源的精选列表. 邀请那些对构建环境感兴趣的人审查并为这个存储库做出贡献.
...
- [Temperature Severity Indicators](https://hudgis-hud.opendata.arcgis.com/datasets/HUD::temperature-severity-indicators/about) - 温度严重性指标数据提取长期温度事件的观测信息，为美国 HUD 提供的住房和社区发展政策和决策提供信息. 
- [Urban Heat Disparities Project](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/1F72FB) - 人口普查区域级温度数据来自 [paper](https://agupubs.onlinelibrary.wiley.com/doi/10.1029/2021EF002016) 使用遥感地表温度测量来探索美国城市供暖负担按人口群体的分布.
- [MACA](https://climate.northwestknowledge.net/MACA/) - MACA 方法是一种统计降尺度方法，用于消除全球气候模型输出的偏差. 
- [IPCC Atlas](https://interactive-atlas.ipcc.ch/) - 一种新颖的工具和数据，用于对许多观测和预测的气候变化信息进行灵活的空间和时间分析，支持第一工作组对第六次评估报告的贡献，包括气候影响驱动因素 (CID) 的区域综合.
- [WUDAPT](https://www.wudapt.org/) - 该数据库旨在提供与全球气候和环境研究相关的城市形态形态和功能的连贯一致的描述和信息.
- [LANDSAT Products](https://www.usgs.gov/landsat-missions/landsat-us-analysis-ready-data) - Landsat 美国分析就绪数据 (ARD) 产品始终按照最高科学标准和处理水平进行处理，可直接用于监测和评估景观变化. 
...
- [Longitudinal Employer-Household Dynamics (LEHD)](https://lehd.ces.census.gov/)  - LEHD 计划结合了这些行政数据以及来自人口普查和调查的数据. 根据这些数据，该程序创建了有关地理和行业详细级别以及不同人口群体的就业、收入和工作流动的统计数据. 附加信息： [On The Map Download Page](https://onthemap.ces.census.gov/).
...
- [Mapping Inequality](https://dsl.richmond.edu/panorama/redlining/)  - 将地理校正的房主贷款公司 (HOLC) 地图分层到美国现代底图上的交互式地图. 这些彩色编码的 HOLC 地图是被称为 *redlining* 的房地产实践的物理体现. 附加信息：许可证：CC BY-NC-SA 4.0，地理覆盖范围：美国 48 个州（1935-1940 年）的主要大都市地区.
...
- [CDC Places](https://www.cdc.gov/places/index.html) - CDC 的合作使地方卫生部门和司法管辖区无论人口规模和城乡状况如何，都能更好地了解其所在地区健康相关结果的负担和地理分布，并协助他们规划公共卫生干预措施.
...
- [NLCD](https://www.usgs.gov/centers/eros/science/national-land-cover-database) - 国家土地覆盖数据库是与多分辨率土地特征联盟 (MRLC) 合作生成的，该联盟是联邦机构的合作伙伴，旨在为美国所有 50 个州和波多黎各生产当前的、全国一致的土地覆盖产品. 
- [Corine Land Cover](https://land.copernicus.eu/pan-european/corine-land-cover) - European Land Cover Database generated by the Land Monitoring Core Service (LMCS) of [Copernicus](https://land.copernicus.eu/)，欧洲地球观测旗舰计划. 
...
参考 (TIGER) 数据库包含从美国人口普查的主地址文件中提取的边界、道路线和边界信息. 
- [Transit Equity Dashboards](https://dashboard.transitcenter.org/) - 由 TransitCenter 发布的七个（并且还在增加）美国城市的交通系统的公平性分析.
...
- [Mapillary](https://www.mapillary.com/)  - 访问来自世界各地的街道级图像和 AI 衍生的地图数据. 通过请求新的报道或使用易于使用的移动应用程序捕获您自己的报道来填补空白.
...
- [FloodFactor](https://firststreet.org/flood-factor/)  - 由第一街基金会提供，该工具可以轻松确定房产是否曾因过去的重大事件而被洪水淹没，目前是否处于危险之中，以及该风险如何随时间变化. 它还可以帮助估算与洪水相关的损害成本.

### Urban Observation

- [Numina](https://numina.co/)  - 利用边缘计算设备测量各种路边活动.  Numina 匿名和汇总地提供街道上的旅行者和物体的体积计数、路径和交通行为.
- [Transoft](https://safety.transoftsolutions.com/)  - Transfoft ITS 安全解决方案利用计算机视觉来识别街道上的“未遂”行为. 这项技术在《规划》杂志上发表 [Art of Learning by Example](https://www.planning.org/planning/2020/oct/art-of-learning-by-example/).
...
- [CityGML](https://www.ogc.org/standards/citygml)  - CityGML 标准为虚拟 3D 城市模型的表示、存储和交换定义了概念模型和交换格式. 它为公民系统提供了一个跨学科的通用操作平台. 
...
### Environmental 
- [EPA Data Standards](https://www.epa.gov/data-standards/epa-data-standards) - 一张图表，其中包含 EPA 采用的各种数据标准的链接.
...
- [ArcGIS Python API](https://developers.arcgis.com/python/api-reference/)  - ArcGIS API for Python 是一个功能强大、现代且易于使用的 Pythonic 库，用于执行 GIS 可视化和分析、空间数据管理和 GIS 系统管理任务，既可以交互运行，也可以使用脚本运行. 该库可以在 ArcGIS 平台中轻松使用 pandas. 
...
- [cityseer-api](https://github.com/benchmark-urbanism/cityseer-api/) - 城市分析的计算工具
- [Geemap](https://github.com/giswqs/geemap) - 用于与 Google 地球引擎、ipyleaflet 和 ipywidgets 交互映射的 Python 包.
- [Geopandas](https://geopandas.org/)  - GeoPandas 是一个开源项目，可以更轻松地在 python 中处理地理空间数据.  GeoPandas 扩展了 pandas 使用的数据类型，以允许对几何类型进行空间操作.
- [Geoplot](https://residentmario.github.io/geoplot/)  - geoplot 是一个高级 Python 地理空间绘图库. 它是对 cartopy 和 matplotlib 的扩展，使映射变得容易：就像 seaborn 用于地理空间. 它具有以下功能：
- [Geosnap](https://github.com/spatialucr/geosnap) - 使用美国人口普查数据进行地理空间和时间邻域分析.
- [global indicators](https://github.com/global-healthy-liveable-cities) - 全球健康和可持续城市指标合作
...
- [MovingPandas](https://anitagraser.github.io/movingpandas/#:~:text=MovingPandas%20is%20a%20Python%20library,movement%20data%20exploration%20and%20analysis.)  - MovingPandas 是一个 Python 库，用于处理基于 Pandas、GeoPandas 和 HoloViz 的运动数据.  MovingPandas 为运动数据探索和分析提供轨迹数据结构和功能.
- [OSMnx](https://github.com/gboeing/osmnx)  - 用于街道网络的 Python. 从 OpenStreetMap 检索、建模、分析和可视化街道网络和其他空间数据.
- [Pandana](https://udst.github.io/pandana/)  - 旨在帮助规划人员计算快速可达性指标或最短路径的城市网络分析库. 附加信息：许可证：AGPL-3.0 许可证：提供者：UrbanSim Inc. (UDST).
...
- [peartree](https://github.com/kuanb/peartree) - 用于将公交数据转换为有向图以进行草图网络分析的库.
...
- [Rasterio](https://github.com/rasterio/rasterio/blob/master/docs/index.rst)  - 地理信息系统使用 GeoTIFF 和其他格式来组织和存储网格化栅格数据集，例如卫星图像和地形模型.  Rasterio 读取和写入这些格式并提供基于 Numpy N 维数组和 GeoJSON 的 Python API.
- [scikit-mobility](https://github.com/scikit-mobility/scikit-mobility) - scikit-mobility：Python 中的移动性分析
...
- [UrbanAccess](https://udst.github.io/urbanaccess/index.html) - 用于计算 GTFS 公交和 OSM 步行网络以进行可访问性分析的工具.
...
- [Urbansprawl](https://github.com/lgervasoni/urbansprawl)  - urbansprawl 项目提供了一个开源框架，用于使用开放数据评估城市蔓延. 它使用 OpenStreetMap (OSM) 数据来计算其庞大的指数，分为可达性、土地利用组合和分散.
- [Xarray-spatial](https://xarray-spatial.org/) - Xarray-Spatial 使用 Numba 实现常见的栅格分析功能，并为栅格分析提供易于安装、易于扩展的代码库.

### R
R 包和相关资源.

- [Foot](https://wpgp.github.io/foot/) -foot 软件包旨在提供一套一致且灵活的工具，用于处理建筑物的 2D 矢量表示和计算城市形态测量.
...
- [Motif](https://cran.r-project.org/web/packages/motif/index.html)  - 描述任何已定义的规则和不规则区域的分类栅格数据的空间模式. 模式使用基于共现矩阵的内置签名进行定量描述，但也允许任何用户定义的函数. 它可以对空间模式执行空间分析，例如搜索、变化检测和聚类 
- [Opentripplanner](https://github.com/opentripplanner) - 一个开源的多模式旅行计划器
- [Raster](https://cran.r-project.org/web/packages/raster/index.html)  - 空间数据的读取、写入、操作、分析和建模. 该包实现了栅格数据和矢量数据操作（如交叉点）的基本和高级功能. 
- [Rgee](https://github.com/r-spatial/rgee) - rgee 是一个 R 绑定包，用于从 R 内部调用 Google Earth Engine API.实现了各种功能以简化与 R 空间生态系统的连接.
- [R Shiny](https://shiny.rstudio.com/) - 使用 R 创建动态的、网络友好的数据可视化.附加信息： [Shiny Tutorial](https://shiny.rstudio.com/tutorial/).
- [Sfnetworks](https://cran.r-project.org/web/packages/sfnetworks/index.html) - 以类和函数的形式提供一种整洁的空间网络分析方法，使网络分析包“tidygraph”和空间分析包“sf”之间能够无缝交互.
- [simplefeatures(sf)](https://cran.r-project.org/web/packages/sf/index.html) - 支持简单特征，一种在 R 中编码空间矢量数据的标准化方法. 
- [Stars](https://cran.r-project.org/web/packages/stars/index.html) - 使用“sf”提供的“GDAL”绑定和“ncmeta”和“RNetCDF”提供的“NetCDF”绑定在“R”中读取、操作、写入和绘制时空数组（光栅和矢量数据立方体）
- [stplanr](https://cran.r-project.org/web/packages/stplanr/index.html)  - 交通规划工具，重点是空间交通数据和非机动模式. 支持常见的交通规划任务，包括：下载和清理交通数据集； 从起点-终点 (OD) 数据创建地理“期望线”； 路由分配，本地和通过接口到路由服务 
...
- [Cesium](https://cesium.com/) - Cesium 是软件应用程序的开放平台，旨在释放 3D 数据的力量.
- [Chart.js](https://www.chartjs.org/)  - 为 Web 和移动设备创建简单（或高级）的数据可视化. 附加信息： [GitHub](https://github.com/chartjs/Chart.js).
- [graphhopper](https://www.graphhopper.com/) - API 路线规划器
...
- [Maptalk.js](https://maptalks.org/)  - Maptalks 通过俯仰和旋转 2D 地图将 2D/3D 地图集成在一起. 借助简单的 2D 地图，为制图、可视化和分析提供更多维度.
...
- [City Energy Analyst](https://cityenergyanalyst.com/) - 一系列 [open-source](https://github.com/architecture-building-systems/CityEnergyAnalyst) 分析和优化城市能源系统的工具. 
...
- [depthmapX](https://www.ucl.ac.uk/bartlett/architecture/research/space-syntax/depthmapx#:~:text=depthmapX%20is%20an%20open%2Dsource,source%20and%20available%20as%20depthmapX.) - depthmapX 是一款针对不同尺度空间网络的开源多平台空间分析软件. 
...
- [Orfeo](https://www.orfeo-toolbox.org/)  - Orfeo ToolBox (OTB) 是一个开源项目，用于最先进的遥感技术. 它建立在开源地理空间社区的肩膀上，可以处理 TB 级的高分辨率光学、多光谱和雷达图像. 
...
- [Urban-pulse](https://github.com/ViDA-NYU/urban-pulse) - Urban Pulse 是一个使用计算拓扑技术来捕捉城市脉搏的框架.

### Public Engagement Tools and Platforms
旨在帮助公众参与和外展活动的软件、工具和平台. 
- [bangthetable](https://www.bangthetable.com/) - 提供对集中社区参与的平台的访问，从而通过一个单一平台轻松捕获、分析和报告全市范围的参与.
- [Citybound](https://aeplay.org/citybound) - 一个城市建设游戏，使用微观模型，生动地模拟了数以百万计的个体相互作用产生的城市有机体.
- [Hello Lamp Post](https://www.hellolamppost.co.uk/) - Hello Lamp Post 是一种专有的文本和基于 SMS 的街头公众参与工具，它使用物理标牌和移动设备来获取市民反馈、改进寻路、推广当地活动和服务，或提供创造性的教育内容.
...
- [Roofpedia](https://github.com/ualsg)  - 城市分析实验室是新加坡国立大学的一个研究小组. 我们的重点是 GIS、3D 城市建模和城市信息学

### Transportation Planning Tools and Platforms
旨在协助运输规划任务和流程的软件、工具和平台. 
- [AequilibraE](http://www.aequilibrae.com/python/latest/overview.html) - AequilibraE 是第一个用于交通建模的综合 Python 包，它旨在提供 Python（NumPy，真的）生态系统中其他开源包不易获得的所有资源.
- [Continuity in Street Networks](http://docs.momepy.org/en/stable/index.html)  - Momepy 是一个用于城市形态定量分析的库 - 城市形态计量学. 它是 PySAL（Python 空间分析库）的一部分，构建在 GeoPandas、其他 PySAL 模块和 networkX 之上.
...
- [Place Syntax Tool](https://www.smog.chalmers.se/pst) - peartree：用于将公交数据转换为有向图以进行草图网络分析的库.
...
- [awesome-sustainable-technology](https://github.com/protontypes/open-sustainable-technology) - 精选的开放技术项目清单，以维持稳定的气候、能源供应和重要的自然资源.
...
这些是其他聚合器，它们汇集了不同的规划相关工具、软件、数据和相关资源.
...
城市和区域规划资源.  (2020) APA 技术部. 取自：来自 https://github.com/APA-Technology-Division
