...
[LIDAR](https://en.wikipedia.org/wiki/Lidar) 是一种遥感传感器，它使用激光以约厘米的精度测量周围环境. 感官数据通常被称为点云，即 3D 或 2D 数据点的集合. 该列表包含硬件、数据集、点云处理算法、点云框架、模拟器等.
...
- [Pioneer](http://autonomousdriving.pioneer/en/3d-lidar/)  - LIDAR 制造商，专注于基于 MEMS 镜面的光栅扫描 LiDAR (3D-LiDAR).  Pioneer 总部位于日本东京.
...
- [Ford Dataset](https://avdata.ford.com/)  - 数据集带有时间戳，包含来自所有传感器、校准值、姿态轨迹、地面真实姿态和 3D 地图的原始数据. 数据与机器人操作系统 (ROS) 兼容.
  - [Paper :newspaper:](https://arxiv.org/pdf/2003.07969.pdf)
  - [GitHub repository :octocat:](https://github.com/Ford/AVData)
- [Audi A2D2 Dataset](https://www.a2d2.audi) - 数据集包含 2D 语义分割、3D 点云、3D 边界框和车辆总线数据.
  - [Paper :newspaper:](https://www.a2d2.audi/content/dam/a2d2/dataset/a2d2-audi-autonomous-driving-dataset.pdf)
- [Waymo Open Dataset](https://waymo.com/open/) - 数据集包含激光雷达和相机数据独立生成的标签，而不仅仅是投影.
- [Oxford RobotCar](https://robotcar-dataset.robots.ox.ac.uk/) - Oxford RobotCar 数据集包含超过 100 次重复通过英国牛津的一致路线，在一年多的时间内捕获. 
...
- [EU Long-term Dataset](https://epan-utbm.github.io/utbm_robocar_dataset/)  - 该数据集是使用我们的机器人汽车（当然是人类驾驶模式）在法国蒙贝利亚尔市中心（用于长期数据）和郊区（用于环形交叉路口数据）收集的，配备多达 11 个异构传感器. 根据法国交通规则，车速被限制在 50 公里/小时.
...
- [Cirrus dataset](https://developer.volvocars.com/open-datasets/cirrus/) 来自 LIDAR 扫描模式非均匀分布的公共数据集，重点是远程. 在这个数据集中使用了 Luminar Hydra LIDAR. 该数据集可在沃尔沃汽车创新门户网站上获得.
...
- [Point Cloud Library (PCL)](http://www.pointclouds.org/) - 流行的高度并行编程库，具有众多工业和研究用例. 
...
- [PyVista](https://docs.pyvista.org/) - 通过可视化工具包的流线型界面进行 3D 绘图和网格分析.
...
- [RangeNet++ :newspaper:](https://www.ipb.uni-bonn.de/wp-content/papercite-data/pdf/milioto2019iros.pdf) - 具有完全卷积网络的快速准确的 LiDAR 语义分割.
...
- [PolarNet :newspaper:](https://arxiv.org/pdf/2003.14032.pdf) - 一种改进的在线 LiDAR 点云语义分割网格表示.
...
- [Frustum PointNets :newspaper:](https://arxiv.org/pdf/1711.08488.pdf) - 从 RGB-D 数据进行 3D 对象检测的 Frustum PointNets.
...
- [LIO-SAM :newspaper:](https://arxiv.org/pdf/2007.00258.pdf) - 通过平滑和映射实现紧密耦合的激光雷达惯性里程计.
...
- [Removert :newspaper:](http://ras.papercept.net/images/temp/IROS/files/0855.pdf) - 移除，然后还原：使用多分辨率范围图像构建静态点云地图.
...
- [CARLA](https://carla.org/)  - 基于虚幻引擎的汽车应用模拟器. 兼容Autoware、百度Apollo和ROS/ROS2.
...
- [LGSVL](https://www.lgsvlsimulator.com/)  - 基于 Unity 引擎的汽车应用模拟器. 兼容Autoware、百度Apollo和ROS/ROS2.
...
- [Semantic Segmentation Editor :octocat:](https://github.com/Hitachi-Automotive-And-Industry-Lab/semantic-segmentation-editor) - 日立汽车和工业实验室的点云和图像语义分割编辑器，点云注释器/标签.
